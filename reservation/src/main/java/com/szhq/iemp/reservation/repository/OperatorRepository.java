...
public interface OperatorRepository extends JpaRepository<Toperator, Integer>, JpaSpecificationExecutor<Toperator> {
    ...
    @Query(value = "select TO1.id,TO1.name, TO2.id as parentId, TO1.device_store_id from t_operator TO1 join t_operator TO2 on TO1.parent_id=TO2.id where TO2.id=?1", nativeQuery = true)
