...
public interface ElectrombileRepository extends JpaRepository<Telectrmobile, Long>, JpaSpecificationExecutor<Telectrmobile> {
    ...
    public List<Telectrmobile> findAllElecsByElecIdIn(List<Long> ids);

    @Query(value = "select * from t_electrmobile where electrmobile_id in (?1) and model_no=?2", nativeQuery = true)
    ...
    public Telectrmobile findByPlateNumberAndOperatorIdsIn(String plateNumber, List<Integer> operatorIds);
    ...
    public List<Telectrmobile> findByImeiIn(List<String> imeis);

    @Query(value = "select imei, plate_number from t_electrmobile where imei in (?1)", nativeQuery = true)
    public List<Map<String, Object>> getPlateNoByImeis(List<String> imeis);
    ...
    public Integer updateViewDate(Date viewDate, Long electrombileId);
    ...
    public List<Map<String, Object>> findAllElecByCriteria(String phone, String realname, String plateNumber, String imei, Date startTime, Date endTime, Pageable pageable);
    ...
    public List<Map<String, Object>> findAllElecByCriteriaAndOperatorIds(String phone, String realname, String plateNumber, String imei, Date startTime, Date endTime, List<Integer> operatorIds, Pageable pageable);
    ...
    @Query(value = "select count(TEU.id) from t_electrmobile_user TEU left join t_electrmobile TE on TEU.electrombile_id=TE.electrombile_id left join t_no_tracker_elec TNTE on TNTE.id=TEU.no_tracker_elec_id left join user U on U.id=TEU.user_id " +
            "where if(?1 is null or ?1='', 1=1, U.phone=?1) and if(?2 is null or ?2='', 1=1, U.name=?2) and if(?3 is null or ?3='', 1=1, TE.plate_number=?3 or TNTE.plate_number=?3) " +
            "and if(?4 is null or ?4='', 1=1, TE.imei=?4) and if(?5 is null and ?6 is null, 1=1, TEU.create_time between ?5 and ?6)", nativeQuery = true)
    public Long countAllElecByCriteria(String phone, String realname, String plateNumber, String imei, Date startTime, Date endTime);

    @Query(value = "select count(TEU.id) from t_electrmobile_user TEU left join t_electrmobile TE on TEU.electrombile_id=TE.electrombile_id left join t_no_tracker_elec TNTE on TNTE.id=TEU.no_tracker_elec_id left join user U on U.id=TEU.user_id " +
            "where if(?1 is null or ?1='', 1=1, U.phone=?1) and if(?2 is null or ?2='', 1=1, U.name=?2) and if(?3 is null or ?3='', 1=1, TE.plate_number=?3 or TNTE.plate_number=?3) " +
            "and if(?4 is null or ?4='', 1=1, TE.imei=?4) and if(?5 is null and ?6 is null, 1=1, TEU.create_time between ?5 and ?6) and TEU.operator_id in (?7)", nativeQuery = true)
    public Long countAllElecByCriteriaAndOperatorIds(String phone, String realname, String plateNumber, String imei, Date startTime, Date endTime, List<Integer> operatorIds);
