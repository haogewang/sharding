...
//	private Lock lock = new ReentrantLock();//基于底层IO阻塞考虑
    ...
    public Object brpop(String key) throws InterruptedException {
        List<byte[]> results = connection.bRPop(0, key.getBytes());
        if (CollectionUtils.isEmpty(results)) {
            return null;
        }
        return redisTemplate.getValueSerializer().deserialize(results.get(1));
    }

    public Object blpop(String key) throws InterruptedException {
