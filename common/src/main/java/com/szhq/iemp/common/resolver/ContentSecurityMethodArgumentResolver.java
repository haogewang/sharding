...
public class ContentSecurityMethodArgumentResolver extends BaseMethodArgumentResolver {
    private static final Logger logger = LoggerFactory.getLogger(ContentSecurityMethodArgumentResolver.class);
    ...
    protected void bindRequestAttributes(WebDataBinder binder, NativeWebRequest nativeWebRequest) throws Exception {
        ...
        while (attributeNames.hasMoreElements()) {
            ...
            if (!attributeName.startsWith(ContentSecurityConstant.ATTRIBUTE_PREFFIX)) {
                continue;
            }
            //获取字段名
            String fieldName = attributeName.replace(ContentSecurityConstant.ATTRIBUTE_PREFFIX, "");
            Field field = null;
            try {
