...
public interface SaleRecordRepository extends JpaRepository<TsaleRecord,Long>, JpaSpecificationExecutor<TsaleRecord> {
    ...
    @Query(value = "select DATE_FORMAT(`update_time`, '%Y-%m-%d') as days,count(distinct imei, if(mode=(1 or 2),true,false)) AS active_count, count(distinct imei,if(mode=0,true,null)) as unactive_count from t_sale_record where operator_id in(?1) and datediff(now(),update_time) <= ?2 GROUP BY days ORDER BY create_time DESC", nativeQuery = true)
    List<Map<String, Object>> saleStatisticByOperatorId(List<Integer> ids, Integer offset);

    @Query(value = "select DATE_FORMAT(`update_time`, '%Y-%m-%d') as days,count(distinct imei, if(mode=(1 or 2),true,false)) AS active_count, count(distinct imei,if(mode=0,true,null)) as unactive_count from t_sale_record where group_id =?1 and datediff(now(),update_time) <= ?2 GROUP BY days ORDER BY create_time DESC", nativeQuery = true)
